...
def accessible_owner_ids():
    ids = [g.user.id]
    links = User_Access.select().where(User_Access.viewer == g.user)
    for link in links:
        ids.append(link.owner.id)
    return ids


def can_view_user(owner):
...
def require_can_view_user(owner):
    if not can_view_user(owner):
        abort(403)
...
def get_accessible_person_or_404(person_id):
    try:
        return Person.get(
            (Person.id == person_id) &
            (Person.user.in_(accessible_owner_ids()))
        )
...
def get_owned_person_or_404(person_id):
    try:
        return Person.get((Person.id == person_id) & (Person.user == g.user))
    except Person.DoesNotExist:
        abort(404)
...
def get_accessible_content_or_404(content_id):
    try:
        return Content.get(
            ...
            (Content.user.in_(accessible_owner_ids()))
        )
...
def get_owned_content_or_404(content_id):
    try:
        return Content.get((Content.id == content_id) & (Content.user == g.user))
    except Content.DoesNotExist:
        abort(404)


def get_accessible_upload_or_404(filename):
    upload = Upload.get_or_none(Upload.filename == filename)
    if not upload:
        abort(404)
    require_can_view_user(upload.owner)
    return upload
...
class SharedUsersEndpoint(Resource):
    ...
    def get(self):
        links = User_Access.select().where(User_Access.viewer == g.user)
        return {"users": [serialize_user_account(link.owner) for link in links]}, 200
...
class MyViewersEndpoint(Resource):
    @require_auth
    def get(self):
        links = User_Access.select().where(User_Access.owner == g.user)
        return {"viewers": [serialize_user_account(link.viewer) for link in links]}, 200
...
class PdfNumPagesEndpoint(Resource):
    @require_auth
    def get(self, filename):
        get_accessible_upload_or_404(filename)
        filepath = os.path.join(app.config["UPLOAD_FOLDER"], filename)
...
class PersonEndpoint(Resource):
    ...
    def post(self):
        data = get_json_or_400(required_fields=[
            ...
            "lastName", "gender", "isDead", "middleName", "maidenName",
            "privacy", "children"
        ])
    ...
    def get(self, id=None):
        if id is None:
            people = Person.select().where(Person.user == g.user)
            return {"people": [serialize_person_summary(p) for p in people.iterator() if privacy_allows(p)]}, 200

        person = get_accessible_person_or_404(id)
        if not privacy_allows(person):
            abort(403)

        content_list = []
        for c in person.content:
            content = c.content
            if content.user.id not in accessible_owner_ids():
                continue
            if not privacy_allows(content):
                continue
        ...
        result = {
            ...
            "parent1": serialize_person_summary(person.parent1_id) if person.parent1_id and can_view_user(person.parent1_id.user) else None,
            "parent2": serialize_person_summary(person.parent2_id) if person.parent2_id and can_view_user(person.parent2_id.user) else None,
            "spouse": serialize_person_summary(person.spouse_id) if person.spouse_id and can_view_user(person.spouse_id.user) else None,
            "children": [
                serialize_person_summary(child)
                for child in Person.select().where(
                    (Person.user == person.user) &
                    ((Person.parent1_id == person) | (Person.parent2_id == person))
                )
                if privacy_allows(child)
            ],
        }
        return result, 200
    ...
    def patch(self, id):
        data = get_json_or_400(required_fields=[
            ...
            "gender", "isDead", "middleName", "maidenName", "privacy", "spouse", "children"
        ])
...
class SharedPeopleEndpoint(Resource):
    @require_auth
    def get(self, owner_id):
        owner = User.get_or_none(User.id == owner_id)
        if not owner:
            abort(404)
        require_can_view_user(owner)
        people = Person.select().where(Person.user == owner)
        return {
            "owner": serialize_user_account(owner),
            "people": [serialize_person_summary(p) for p in people.iterator() if privacy_allows(p)],
        }, 200


api.add_resource(SharedPeopleEndpoint, "/users/<int:owner_id>/people")
...
class ContentEndpoint(Resource):
    ...
    def post(self):
        data = get_json_or_400(required_fields=[
            "privacy", "type", "date", "notes", "title", "file", "location", "dateUnknowns", "people"
        ])
    ...
    def get(self, id=None):
        if id is None:
            contents = Content.select().where(Content.user == g.user)
            return {"content": [serialize_content_summary(c) for c in contents.iterator() if privacy_allows(c)]}, 200
        ...
        for p in content.person:
            person = p.person
            if person.user.id not in accessible_owner_ids():
                continue
            if not privacy_allows(person):
                continue
            person_list.append(serialize_person_summary(person))
    ...
    def patch(self, id):
        data = get_json_or_400(required_fields=[
            "privacy", "type", "date", "notes", "title", "location", "dateUnknowns", "people"
        ])
...
class SharedContentEndpoint(Resource):
    @require_auth
    def get(self, owner_id):
        owner = User.get_or_none(User.id == owner_id)
        if not owner:
            abort(404)
        require_can_view_user(owner)
        contents = Content.select().where(Content.user == owner)
        return {
            "owner": serialize_user_account(owner),
            "content": [serialize_content_summary(c) for c in contents.iterator() if privacy_allows(c)],
        }, 200


api.add_resource(SharedContentEndpoint, "/users/<int:owner_id>/content")
...
class GenderEndpoint(Resource):
    def get(self):
        return {
            "genders": [{"id": gender.id, "name": gender.name} for gender in Gender.select().iterator()]
        }, 200
...
def load_prolog_facts(prolog_engine, owner):
    try:
    ...
    for person in Person.select().where(Person.user == owner):
        if not privacy_allows(person):
            continue
        ...
        if person.parent1_id and person.parent1_id.user.id == owner.id:
            prolog_engine.assertz(f"parent({person.parent1_id.id}, {person.id})")
        if person.parent2_id and person.parent2_id.user.id == owner.id:
            prolog_engine.assertz(f"parent({person.parent2_id.id}, {person.id})")
        if person.spouse_id and person.spouse_id.user.id == owner.id:
...
class QueryRelationshipEndpoint(Resource):
    ...
    def post(self):
        ...
        if person1.user.id != person2.user.id:
            abort(400, description="Relationship queries must stay inside one account.")

        allowed_relationships = [
            "parent", "spouse", "sibling", "brother", "sister", "grandparent",
            "grandfather", "grandmother", "child", "son", "daughter", "ancestor",
            "descendant", "cousin", "aunt", "uncle", "niece", "nephew",
        ]
        ...
        load_prolog_facts(prolog_engine, person1.user)
...
class FamilyTreeEndpoint(Resource):
    ...
    def get(self, person_id):
        ...
        if not privacy_allows(root_person):
            abort(403)

        people, parent_edges, spouse_edges = collect_family(root_person)

        # Safety filter so shared viewers only see this account's allowed people.
        people = {
            pid: person
            for pid, person in people.items()
            if person.user.id == root_person.user.id and privacy_allows(person)
        }
        parent_edges = [(a, b) for a, b in parent_edges if a in people and b in people]
        spouse_edges = [(a, b) for a, b in spouse_edges if a in people and b in people]
        ...
        positions = layout_full_tree(
            people, generation, two_parent_families, single_parent_families,
            node_width=NODE_WIDTH, spouse_gap=SPOUSE_GAP, level_gap=V_SPACING,
            root_gap=40, sibling_gap=40,
        )
